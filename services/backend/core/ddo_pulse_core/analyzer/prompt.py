...
# ---------------------------------------------------------------------------
# 双维度评分 Prompt 模板（relevance + novelty）
# ---------------------------------------------------------------------------

DUAL_SCORE_PROMPT_TEMPLATE = """你是一位中文技术博客策展编辑。请阅读以下文章信息，判断是否值得推荐给中文读者，并输出**仅包含一个合法 JSON 对象**（不要用 markdown 代码块；字符串内若有双引号须写成 \\"）。
